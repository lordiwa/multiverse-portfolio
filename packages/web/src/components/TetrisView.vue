...
<template>
  <div :class="['tetris-view', theme.containerClass]">
    ...
    <div class="tetris-header">
      <button class="back-btn" :disabled="gameRunning && !paused" @click="goBack">
    ...
    <div class="tetris-content">
      ...
      <div class="game-container">
        ...
        <div class="game-board-container">
          ...
          <div v-if="!gameRunning" class="game-overlay">
            <div class="overlay-content">
              ...
              <div v-if="score > 0 && !scoreSubmitted" class="leaderboard-form">
                <input
                  v-model="playerName"
                  type="text"
                  placeholder="Enter your name"
                  maxlength="20"
                  :class="['name-input', theme.sectionStyle]"
                  @keyup.enter="submitScore"
                />
                <button
                  :class="['btn-submit', theme.sectionStyle]"
              ...
              <!-- Submission feedback -->
              <div v-if="scoreSubmitted" class="submission-feedback">
                <span class="success-icon">✓</span>
                <span>Score submitted to leaderboard!</span>
              </div>
              <div v-if="submitError" class="submission-error">
                <span>{{ submitError }}</span>
              </div>

              <button :class="['btn-start', theme.sectionStyle]" @click="startGame">
        ...
        <div class="game-stats">
          ...
          <!-- Next Piece Preview -->
          <div v-if="nextPiece" :class="['next-piece-card', theme.sectionStyle]">
            <h4>Next Piece</h4>
            <canvas
              ref="nextCanvas"
              width="120"
      ...
      <div class="mobile-controls">
        ...
        <div class="control-row">
          <button :class="['control-btn', theme.sectionStyle]" :disabled="!gameRunning || paused" @click="rotatePieceHandler">
            ↻<span class="btn-label">Rotate</span>
          </button>
          <button :class="['control-btn', theme.sectionStyle]" :disabled="!gameRunning || paused" @click="hardDrop">
            ⬇<span class="btn-label">Drop</span>
          </button>
        </div>
        <div class="control-row">
          <button :class="['control-btn', theme.sectionStyle]" :disabled="!gameRunning || paused" @click="() => movePiece(-1, 0)">
            ←<span class="btn-label">Left</span>
          </button>
          <button :class="['control-btn', theme.sectionStyle]" :disabled="!gameRunning || paused" @click="() => movePiece(0, 1)">
...
const submitScore = async () => {
  if (!playerName.value.trim() || submitting.value || scoreSubmitted.value) return
  ...
  try {
    ...
    if (response.ok) {
      ...
      submitError.value = data.error || 'Failed to submit score'
    }
  } catch {
