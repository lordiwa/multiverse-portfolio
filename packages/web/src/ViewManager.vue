<template>
  <div class="view-manager" :class="currentTheme?.containerClass">
    <!-- Synthwave Loader -->
    <SynthwaveLoader v-if="isLoading" @loaded="onLoaded" />

    <!-- Synthwave Background Elements (only for Current Universe) -->
    <div v-if="!isLoading && selectedCareer === 'current'" class="synthwave-bg">
      <!-- Enhanced Grid Floor -->
      <div class="synth-grid"></div>

      <!-- Synthwave Sun with lines -->
      <div class="synth-sun">
        <div class="sun-body"></div>
        <div class="sun-lines">
          <div v-for="n in 8" :key="n" class="sun-line"></div>
        </div>
      </div>

      <!-- Mountain Silhouettes -->
      <svg class="synth-mountains" viewBox="0 0 1920 400" preserveAspectRatio="none">
        <!-- Back layer mountains (darker) -->
        <polygon class="mountain mountain-back-1" points="0,400 300,120 600,400" />
        <polygon class="mountain mountain-back-2" points="400,400 750,80 1100,400" />
        <polygon class="mountain mountain-back-3" points="900,400 1250,100 1600,400" />
        <polygon class="mountain mountain-back-4" points="1400,400 1700,140 1920,400" />
        <!-- Front layer mountains -->
        <polygon class="mountain mountain-front-1" points="0,400 200,180 450,400" />
        <polygon class="mountain mountain-front-2" points="350,400 600,150 900,400" />
        <polygon class="mountain mountain-front-3" points="750,400 1050,130 1350,400" />
        <polygon class="mountain mountain-front-4" points="1200,400 1500,160 1750,400" />
        <polygon class="mountain mountain-front-5" points="1600,400 1800,200 1920,400" />
      </svg>
    </div>

    <!-- Main Portfolio View -->
    <div v-if="!isLoading && currentView === 'portfolio'" :class="['portfolio-view', { 'transitioning': isTransitioning }]">
      <!-- Universe Switcher -->
    ...
    <!-- Email Contact View -->
    <EmailContactView
      v-else-if="!isLoading && currentView === 'email'"
      :theme="currentTheme"
      :career="selectedCareer"
    ...
    <!-- Mad Libs Game View -->
    <MadLibsView
      v-else-if="!isLoading && currentView === 'madlibs'"
      :theme="currentTheme"
      :career="selectedCareer"
      @back="backToPortfolio"
    />
    ...
    <!-- Tetris Game View -->
    <TetrisView
      v-else-if="!isLoading && currentView === 'tetris'"
      :theme="currentTheme"
      :career="selectedCareer"
      @back="backToPortfolio"
    />
...
import SynthwaveLoader from './components/SynthwaveLoader.vue'
...
:root {
  ...
  /* Background colors */
  --synth-bg-dark: #0d0221;
  --synth-bg-mid: #1a0533;
  --synth-bg-purple: #2d1b4e;

  /* Glassmorphism with synthwave twist */
  --glass-bg: rgba(13, 2, 33, 0.7);
  --glass-border: rgba(0, 255, 249, 0.2);
  --glass-blur: 20px;
  --glass-shadow: 0 8px 32px rgba(255, 0, 255, 0.2);

  /* Neon glow intensities */
  --neon-glow-sm: 0 0 10px;
  --neon-glow-md: 0 0 20px;
  --neon-glow-lg: 0 0 40px;
  ...
  --tech-primary: #00fff9;
  --tech-secondary: #05d9e8;
  --tech-rgb: 0, 255, 249;

  --artist-primary: #ff2a6d;
  --artist-secondary: #ff00ff;
  --artist-rgb: 255, 42, 109;

  --explorer-primary: #05d9e8;
  --explorer-secondary: #00fff9;
  --explorer-rgb: 5, 217, 232;

  --performer-primary: #ff00ff;
  --performer-secondary: #ff6b35;
  --performer-rgb: 255, 0, 255;

  --gamer-primary: #7b2cbf;
  --gamer-secondary: #00fff9;
  --gamer-rgb: 123, 44, 191;

  --fantasy-primary: #9d4edd;
  --fantasy-secondary: #f9f871;
  --fantasy-rgb: 157, 78, 221;
}

/* Synthwave card base class */
.modern-card {
...
/* Synthwave grid background overlay */
.synthwave-grid {
  position: fixed;
  top: 0;
  left: 0;
...
.modern-skill-badge:hover {
  background: linear-gradient(135deg, var(--synth-cyan), var(--synth-magenta));
  color: white;
  transform: translateY(-2px) scale(1.05);
  box-shadow:
    0 0 20px var(--synth-cyan),
    0 0 40px var(--synth-magenta);
...
.view-manager::after {
  ...
  background:
    ...
    repeating-linear-gradient(
      ...
      transparent,
      transparent 49px,
      rgba(0, 255, 249, 0.3) 49px,
      rgba(0, 255, 249, 0.3) 51px,
      transparent 51px
    ),
    repeating-linear-gradient(
      0deg,
      transparent,
      transparent 49px,
      rgba(255, 0, 255, 0.2) 49px,
      rgba(255, 0, 255, 0.2) 51px,
      transparent 51px
    );
...
.synth-grid {
  ...
  background:
    ...
    linear-gradient(
      to bottom,
      transparent 0%,
      rgba(0, 255, 249, 0.03) 20%,
      rgba(0, 255, 249, 0.08) 100%
    ),
    /* Vertical lines (cyan) */
    repeating-linear-gradient(
      90deg,
      transparent,
      transparent 78px,
      rgba(0, 255, 249, 0.5) 78px,
      rgba(0, 255, 249, 0.5) 82px,
      transparent 82px
    ),
    ...
    repeating-linear-gradient(
      ...
      transparent,
      transparent 78px,
      rgba(255, 0, 255, 0.4) 78px,
      rgba(255, 0, 255, 0.4) 82px,
      transparent 82px
    );
  background-size: 100% 100%, 80px 80px, 80px 80px;
  transform: perspective(400px) rotateX(65deg);
  transform-origin: center top;
  animation: synthGridScroll 8s linear infinite;
}

@keyframes synthGridScroll {
  0% { background-position: 0 0, 0 0, 0 0; }
  100% { background-position: 0 0, 0 80px, 0 80px; }
}
...
.sun-line:nth-child(1) { top: 5%; }
.sun-line:nth-child(2) { top: 15%; height: 8px; }
.sun-line:nth-child(3) { top: 28%; height: 10px; }
.sun-line:nth-child(4) { top: 42%; height: 12px; }
.sun-line:nth-child(5) { top: 56%; height: 14px; }
.sun-line:nth-child(6) { top: 70%; height: 16px; }
.sun-line:nth-child(7) { top: 84%; height: 18px; }
.sun-line:nth-child(8) { top: 96%; height: 20px; }
...
.mountain {
  fill: #0d0221;
  stroke-width: 2;
  filter: drop-shadow(0 0 10px var(--mountain-glow, #ff00ff));
}

/* Back layer - darker, subtle glow */
.mountain-back-1,
.mountain-back-2,
.mountain-back-3,
.mountain-back-4 {
  ...
  --mountain-glow: rgba(255, 0, 255, 0.3);
}

/* Front layer - more vibrant */
.mountain-front-1 {
  stroke: #ff00ff;
  --mountain-glow: #ff00ff;
  animation: mountainPulse 4s ease-in-out infinite;
}

.mountain-front-2 {
  stroke: #00fff9;
  --mountain-glow: #00fff9;
  animation: mountainPulse 4s ease-in-out infinite 0.5s;
}

.mountain-front-3 {
  stroke: #ff00ff;
  --mountain-glow: #ff00ff;
  animation: mountainPulse 4s ease-in-out infinite 1s;
}

.mountain-front-4 {
  stroke: #00fff9;
  --mountain-glow: #00fff9;
  animation: mountainPulse 4s ease-in-out infinite 1.5s;
}

.mountain-front-5 {
  stroke: #ff00ff;
  --mountain-glow: #ff00ff;
  animation: mountainPulse 4s ease-in-out infinite 2s;
}

@keyframes mountainPulse {
  0%, 100% {
    filter: drop-shadow(0 0 10px var(--mountain-glow));
    stroke-opacity: 1;
  }
  50% {
    filter: drop-shadow(0 0 25px var(--mountain-glow)) drop-shadow(0 0 50px var(--mountain-glow));
    stroke-opacity: 0.8;
  }
}
...
.universe-switcher {
  ...
  gap: 6px;
  z-index: 1000;
  background: linear-gradient(135deg, rgba(13, 2, 33, 0.9), rgba(45, 27, 78, 0.8));
...
.api-status-btn:hover {
  background: linear-gradient(135deg, rgba(45, 27, 78, 0.95), rgba(123, 44, 191, 0.8));
  border-color: var(--synth-magenta);
  transform: translateY(-2px);
  box-shadow:
    0 6px 25px rgba(0, 0, 0, 0.5),
    0 0 30px rgba(255, 0, 255, 0.4);
  color: var(--synth-magenta);
  text-shadow: 0 0 15px var(--synth-magenta);
}

.api-status-btn .status-dot {
  width: 10px;
  height: 10px;
  border-radius: 50%;
  background: #ff2a6d;
  box-shadow: 0 0 10px #ff2a6d, 0 0 20px #ff2a6d;
  transition: all 0.3s ease;
}

.api-status-btn .status-dot.online {
...
@keyframes neonPulse {
  ...
  50% {
    transform: scale(1.2);
    box-shadow: 0 0 15px var(--synth-cyan), 0 0 30px var(--synth-cyan), 0 0 45px var(--synth-cyan);
  }
}

.api-status-btn .status-text {
  color: inherit;
}

/* Synthwave Universe Buttons */
.universe-btn {
  padding: 12px 24px;
  border: none;
...
.universe-btn:first-child::before {
  ...
  bottom: -2px;
  background: linear-gradient(45deg,
    var(--synth-cyan), var(--synth-magenta), var(--synth-pink), var(--synth-cyan));
  border-radius: 46px;
  z-index: -1;
  filter: blur(8px);
  opacity: 0.4;
